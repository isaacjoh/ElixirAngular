...
var channel = _socket2.default.channel("walls:" + window.topic, {});
...
WallApp.controller("TweetsController", function ($scope) {
  ...
  this.listen_to_tweets = function () {
    ...
    channel.on("new_tweet", function (message) {
      if (!findById(message.id, _this.tweets)) {
        _this.tweets.push(message);
        if (_this.tweets.length > 20) {
          _this.tweets.shift();
        }
        $scope.$apply();
      }
...
WallApp.controller("MessagesController", function ($scope) {
  ...
  this.listen_to_chat = function () {
    ...
    channel.on("new_msg", function (message) {
      display_message({ body: message.body, user: message.user }, _this2.messages);
      $scope.$apply();
    });
  };
  ...
  this.send_message = function (keyEvent) {
    if (keyEvent.which === 13) {
      ...
      channel.push("new_msg", { body: this.message_box, user: this.user }).receive("ok", function (payload) {
